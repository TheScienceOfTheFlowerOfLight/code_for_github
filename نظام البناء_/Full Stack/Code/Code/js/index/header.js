...
//* ادوات -------------------------------------------
    ...
    // خرائط
        /* 
            https://www.openstreetmap.org/#map=6/26.80/30.25
...
//* ملف -----------------------------------------
    /* 
        EX: index.js
        Code\js
        انشأه بنفسى  JS  يوضع مجلداتها و ملفاتها فى مجلد اسمه 
        انشئ مجلد بنفس اسمه HTML لكل ملف 
        يكون به ملفات لاجزاء الصفحة الرئيسية التى سأنشئ عمليات بها
    */
...
//* CSSOM --------------------------------------------
/* 
    لعنصر ، أو تعدل عليها css تعرض قيم لخصائص  method أو  property   هو أى
    string  كل القيم التى تعرضها تكون من نوع بيان
...
//* التخزين ----------------------------------------------
    ...
    //? المتغيرات
        // شروط كتابة اسم المتغيير
            /* مثل شروط كتابة اسم الأمر */
    ...
    //? الدوال
        ...
        // ()=>{}   // e=> e+11   // (a,s,d)=> a+b+c   //اسمها دالة السهم

        /* 
            return
            اكتبها لو كنت عايز الدالة ترجع ببيان
            لا تكتبها لو عايز الدالة تنفذ عملية فقط ، دون ان ترجع ببيان 
        */

        f2(()=>11,1); /* الدالة التى تكنب كوسيطة ، تسمى دالة مضمنة */
    ...
    //? browser for FE
        /* لتخزين بيانات فى المتصفح الخاص بالمستخدم */
    ...
    //? browser for BE
        /* لتخزين بيانات فى المتصفح الخاص بالمستخدم */
        document.cookie = `c=11;max-age=${3*365*24*60*60*1000};domain=w.com;path=/;sameSite=Strict`;//Lax
...
/* 
    ! true عندما يتحقق الشرط ، اى قيمته المنطقية تساوى 
    ? if يتم تطبيق الكود المجهز لذلك ثم يخرج من جملة الشرط 
    ! false اذا كان قيمة الشرط المنطقية تساوى
    ? (else if) ينتقل  لامر الشرط التالى 
    ! false اذا كان كل القيم المنطقية للشروط فى اوامر الشرط تساوى 
    ? (else) يطبق الكود المكتوب فى الامر 

    افضل استعماله فى حالة وجود كود كبير او فى حالة كثرة الخيارات
*/
...
/* افضلها عندما ارغب بتنفيذ أمر حسب قيمة مخزن*/
...
/* 
    do ينفذ الكود فى
    يكرر تنفيذ الكود ، وهكذا الا ان لا يتحقق الشرط while ثم اذا تحقق الشرط فى 
*/
...
for (let i in opjo){
    ...
    if (vv){
        continue;/* انهى هذه المرة بدأا من هنا ، و انتقل للمرة التالية مباشرتا */
    }

    if (vv == 11){
        break;/* انهى التكرار نهائيا بدأا من هنا */
    }
}
/* 
    متى يستخدم ؟
...
clearInterval(t);
/* 
    clearInterval و setInterval يجب ان يكون المتغيير فى نطاق يحتوى على الاثنين
*/
